...
def bbox_focal_shape_iou(box1, box2, xywh=True, scale=0.5, gamma=0.5, alpha=2.0, eps=1e-7):
    """
    Focal-Shape-IoU (FSIoU) - Designed for SODA10M Small Object Detection
    结合了 Shape-IoU 的几何约束与 Focaler-IoU 的难样本挖掘能力。
    
    Args:
        scale (float): Shape-IoU 的尺度因子，小目标建议 0.5。
        gamma (float): Focaler 的聚焦因子，控制对难样本的关注度 (建议 0.5)。
        alpha (float): Shape 惩罚的权重 (建议 1.5 - 2.0，强迫回归形状)。
    """
    # --- 1. 基础坐标转换 (保持不变) ---
    if xywh: 
        (x1, y1, w1, h1), (x2, y2, w2, h2) = box1.chunk(4, -1), box2.chunk(4, -1)
    ...
    # 距离惩罚
    center_dist = ((b2_x1 + b2_x2 - b1_x1 - b1_x2) ** 2 + (b2_y1 + b2_y2 - b1_y1 - b1_y2) ** 2) / 4
    distance_term = hh * center_dist / c2
    
    # 形状惩罚 (这里我们用了更激进的指数惩罚)
    omiga_w = hh * torch.abs(w1 - w2) / torch.max(w1, w2)
    omiga_h = ww * torch.abs(h1 - h2) / torch.max(h1, h2)
    shape_term = torch.pow(1 - torch.exp(-1 * omiga_w), 4) + torch.pow(1 - torch.exp(-1 * omiga_h), 4)

    # --- 4. Focaler 调制 (难样本挖掘) ---
    # 创新点: 不直接用 iou，而是用 "重构后的 IoU"
    # 当 IoU 较小时，focal_iou 会变得更小，从而拉大 Loss，产生更大梯度
    focal_iou = torch.pow(iou, gamma)

    # --- 5. 最终融合 ---
    # Loss = 1 - (Focal_IoU - 距离惩罚 - alpha * 形状惩罚)
    # alpha 加大，强制网络去修形状
    return focal_iou - distance_term - alpha * shape_term
