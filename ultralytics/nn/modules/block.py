...
# ================== GSConv Modules Start ==================
# 确保文件头部引入了: from .conv import Conv, DWConv, autopad
...
class VoVGSCSP(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):
        ...
        self.cv1 = Conv(c1, c_, 1, 1)
        self.cv2 = Conv(c1, c_, 1, 1)
        # 修复点：移除了 e=1.0，因为 GSBottleneck 不接收 e
        self.m = nn.Sequential(*(GSBottleneck(c_, c_) for _ in range(n)))
        # 输出融合：根据 Slim-neck 原理，这里 concat(x2, x3, x1) 是 3 路
        self.cv3 = Conv(c_ * 2 + c_, c2, 1)
